...
public class EditarAluno extends SelectorComposer<Component> {
	...
	private void atualizar() {
		...
		Include include = (Include) Selectors
				.iterable(cbx_aluno.getPage(), "#mainInclude")
				.iterator().next();
		include.setSrc(null);
	...
	public void criaLista() {
		...
		ListModelList<Aluno> alunoModel = new ListModelList<Aluno>(
				alunos);
		// cria model
		listar_aluno.setModel(alunoModel);
	...
	public void edita() {
		...
		aluno.setCpf(cbx_aluno.getSelectedItem().getLabel().split("-")[0].trim());
		aluno.setMatricula(matricula.getValue());

		if (alunoDAO.updateOne(aluno) > 0) {
