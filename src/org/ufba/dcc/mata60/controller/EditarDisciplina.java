...
public class EditarDisciplina extends SelectorComposer<Component> {
	...
	private void atualizar() {
		...
		Include include = (Include) Selectors
				.iterable(cbx_departamento.getPage(), "#mainInclude")
				.iterator().next();
		include.setSrc(null);
	...
	public void doAfterCompose(Component comp) throws Exception {
		...
		cbx_disciplina.setModel(ListModels.toListSubModel(new ListModelList<>(
				obtemDisciplinas())));
		cbx_departamento.setModel(ListModels
	...
	public void edita() {
		...
		disciplina.setCod(cbx_disciplina.getSelectedItem().getLabel()
				.split("-")[1].trim());
		disciplina.setDepartamento_cod((int) Integer.valueOf(cbx_departamento
				.getSelectedItem().getLabel().split("-")[1].trim()));
